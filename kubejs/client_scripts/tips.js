ItemEvents.tooltip(event => {
    ...
    event.add("gtceu:fission_reactor", ["反应堆在运行前会获得最大并行数量为燃料组件数量的并行", "反应堆在运行过程中会根据配方产热","如果温度高于1500K，反应堆将会损坏，损坏达到100%后反应堆爆炸", "在运行过程中提供冷却液并根据不同冷却液的冷却液系数来控制温度", "冷却液系数：蒸馏水：200，钠钾合金：40", "反应堆冷却有如下参数：", "最低冷却液需求量和最高冷却液供给量", "最低需求量=配方产热x实际并行数量x当前温度/1500", "最高供给量=冷却组件x8", "当供给量>=需求量时达到消耗冷却液条件，消耗提供的冷却液，消耗量为需求量x冷却液系数，并阻止反应堆升温", "如果供给量是需求量的n倍，则执行特殊超频，超频次数为n", "再次消耗需求量的冷却液，并减少1秒时间，如果无法供给冷却液则中断超频，如果进度到达100%则中断超频并消耗一次需求量的冷却液使温度降低1K", "由蒸馏水作为冷却液时将产生蒸汽，产生量：消耗量xmin(160,160/(1.4^(373-温度)))", "由钾钠合金作为冷却液时产生热钾钠合金，产生量=消耗量", "无论反应堆是否具有消耗冷却液的条件都能执行配方","反应堆停止工作后温度将每秒降低1K"])
